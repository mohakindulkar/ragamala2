<script>
    ...
    $: if (hierarchyNode && $activeRaga && $baithakInstruments) {
        ...
        if (targetRaga) {
            ...
            const node = allNodesFlat.find(n => n.data.name === targetRaga.name);
                ...
                if (!$baithakInstruments.sitar && !$baithakInstruments.flute) {
                    ...
                    if (name === 'bhairav') {
                        ...
                        if (needsPakhawaj && !$baithakInstruments.pakhawaj) {
                            baithakInstruments.update(b => ({ ...b, pakhawaj: true, tabla: false }));
                        } else if (!needsPakhawaj && $baithakInstruments.pakhawaj) {
                            baithakInstruments.update(b => ({ ...b, pakhawaj: false, tabla: true }));
                        }
                    }
