<script>
    ...
    $effect(() => {
        ...
        const activeTime = timeData.find(t => hour >= t.startHour && hour < t.endHour);
        ...
        document.documentElement.style.setProperty('--season-color', activeSeason.color);
        document.documentElement.style.setProperty('--time-color', activeTime.color || '#1a1a1a');
        document.documentElement.style.setProperty('--season-time-gradient', `radial-gradient(${activeTime.color}, ${activeSeason.color})`);
        
        if (activeTime.id === 'night') {
            // Force deep blue and violet during Ratri (Night)
            document.documentElement.style.setProperty('--season-color-1', '#050515'); // Deepest Black-Blue
            document.documentElement.style.setProperty('--season-color-2', '#1a1a4a'); // Midnight Blue
            document.documentElement.style.setProperty('--season-color-3', '#2e1a4a'); // Night Violet
            document.documentElement.style.setProperty('--season-color-4', '#101030'); // Shadow Blue
            document.documentElement.style.setProperty('--season-color-5', '#1a0a2a'); // Shadow Violet
        } else {
            document.documentElement.style.setProperty('--season-color-1', `var(--${activeSeason.name.toLowerCase()}-1)`);
            document.documentElement.style.setProperty('--season-color-2', `var(--${activeSeason.name.toLowerCase()}-2)`);
            document.documentElement.style.setProperty('--season-color-3', `var(--${activeSeason.name.toLowerCase()}-3)`);
            document.documentElement.style.setProperty('--season-color-4', `var(--${activeSeason.name.toLowerCase()}-4)`);
            document.documentElement.style.setProperty('--season-color-5', `var(--${activeSeason.name.toLowerCase()}-5)`);
        }
        document.documentElement.style.setProperty('--damask', `var(--damask-${activeSeason.name.toLowerCase()})`);
...
<!--        <div class="corner top-left cutout">
    ...
    <div class="widget-layer">
        ...
        <div class="audio-container"><AudioEngine /></div>
    </div>
...
<style>
    ...
    .translate-container, .audio-container {
        pointer-events: auto; /* Reactivate clicks just for the buttons */
        position: absolute;
    }

    /* Position your widgets as you like */
    .translate-container { top: 20px; right: 20px; }
    .audio-container { bottom: 20px; right: 20px; }
...
/*    !* Optional: Alter frame border color on the Calendar page *!
    ...
    .corner {
        ...
        height: 100px;
        background-color: var(--accent-color); /* Placeholder for arabesque corner image */
    ...
    .border-top    { top: 0; height: 60px; }
    .border-bottom { bottom: 0; height: 60px; }
    ...
    .border-left  { left: 0; }
    .border-right { right: 0; width: 60px; }
